...
namespace AutoPublishProject {
    ...
    internal class Program {
        static void Main(string[] args) {
            ...
            try {
                ...
                while (websiteChoice != 1 && websiteChoice != 2 && websiteChoice != 3) {
                    ...
                    Console.WriteLine("1. Qxy.PdmPortal");
                    Console.WriteLine("2. Qxy.PlatformProductPortal");
                    Console.WriteLine("3. PlatformSyn");
                    Console.Write("请输入选择 (1、2 或 3): ");
                    string input = Console.ReadLine();
                    if (int.TryParse(input, out websiteChoice) && (websiteChoice == 1 || websiteChoice == 2 || websiteChoice == 3)) {
                        break;
                    }
                ...
                switch (websiteChoice) {
                    ...
                    case 3:
                        ...
                        publishProject = new PublishProject {
                            ...
                            ProjectPath = @"D:\QXYSVN\trunk\Qxy.SchedulingServer.PlatformSyn\Qxy.SchedulingServer.PlatformSyn\Qxy.SchedulingServer.PlatformSyn.csproj",
                            PublishProfilePath = string.Empty,
                            PublishDir = @"D:\QXYSVN\trunk\Qxy.SchedulingServer.PlatformSyn\Qxy.SchedulingServer.PlatformSyn\bin\Debug\Deploy",
                            DeploymentDir = @"D:\RemoteFB\FBPlatformSyn",
                            SolutionName = "Qxy.SchedulingServer.PlatformSyn",
                            IsWebProject = false
                        };
                        Console.WriteLine("您选择了发布PlatformSyn项目");
                        break;
                    default:
                        throw new Exception("没有此选择");
                }
        ...
        static void PublishWebsite(PublishProject publishProject) {
            ...
            if (!File.Exists(publishProfilePath) && publishProject.IsWebProject) {
            ...
            try {
                ...
                if (!Directory.Exists(publishDir)) {
                    ...
                    Console.WriteLine($"发布目录已存在: {publishDir}");
                }

                if (!hasError) {
                    // 使用MSBuild执行操作
                    Console.WriteLine($"使用MSBuild执行操作...");
                    ...
                    // 根据是否是网站项目设置不同的参数
                    if (publishProject.IsWebProject) {
                        // 网站项目：执行发布操作
                        Console.WriteLine($"发布配置: {publishProfilePath}");
                        psi.Arguments = $"\"{projectPath}\" /p:DeployOnBuild=true /p:PublishProfile=\"{publishProfilePath}\" /p:Configuration=Release /p:publishUrl=\"{publishDir}\"";
                        Console.WriteLine("开始执行MSBuild发布命令...");
                    } else {
                        // 非网站项目：执行构建操作，并指定输出目录
                        psi.Arguments = $"\"{projectPath}\" /p:Configuration=Debug /p:OutputPath=\"{publishDir}\"";
                        Console.WriteLine("开始执行MSBuild构建命令...");
                        Console.WriteLine($"输出目录: {publishDir}");
                    }
                    ...
                    try {
                        using (Process process = Process.Start(psi)) {
                            ...
                            if (!process.WaitForExit(600000)) {
                                ...
                                if (process.ExitCode != 0) {
                                    Console.WriteLine($"操作失败: {errorOutput}");
